  ...
  return (
    <Grid2 container spacing={3}>
      {diseases.map(({ title, options }) => (
        <Grid2 item size={{xs: 12, sm: 6, md: 3}} key={title}>
          ...
          {options.map((option) => (
            <Button
              ...
              sx={{
                ...
                padding: "10px",
                borderRadius: "25px",
                border: selectedOptions.includes(option) ? "1px solid #00A359" : "1px solid #ddd",
                backgroundColor: selectedOptions.includes(option) ? "#E8F5E9" : "#FFFFFF",
                color: "black",
              }}
            >
              {selectedOptions.includes(option) ? (<CheckCircleIcon color="primary" fontSize="small" />) : (<CircleOutlinedIcon color="action" fontSize="small" />)}
