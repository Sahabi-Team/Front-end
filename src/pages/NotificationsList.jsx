...
export default function NotificationsList() {
  ...
  const handleWriteProgram = async (notif) => {
    ...
    try {
      ...
      navigate(`/createworkoutplan/${notif.trainee_info.trainee_username}`);
    }
    catch (error) {
            ...
            {notifications.map((notif, index) => (
              <React.Fragment key={index}>
                <ListItem sx={{my: 1, justifyContent: 'space-between'}}>
                  ...
                  {!notif.is_read && <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleWriteProgram(notif)} sx={{borderRadius: 2, py: "2px"}}>نوشتن برنامه</Button>}
                </ListItem>
