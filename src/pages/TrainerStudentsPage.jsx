...
const TrainerStudentsPage = () => {
  ...
  const handleCreateProgramClick = (username) => {
    navigate(`/createworkoutplan/${username}`);
  };
    ...
    const fetchTrainees = async () => {
      try {
        ...
        const response = await api.get(
          `${config.API_BASE_URL}/api/trainer/my-trainees/`
        );
  ...
  return (
    <MainLayout>
      ...
      <Box sx={{ display: "flex", flexDirection: "column" }}>
        <Typography
          variant="body2"
          sx={{ color: "#666", mb: 2, textAlign: "left" }}
        >
          {loading
            ? "در حال بارگذاری..."
            : error
            ? "خطا در بارگذاری"
            : filteredStudents.length.toLocaleString("fa-IR")}{" "}
          {!loading && !error && "نفر پیدا شد"}
        </Typography>
        <Divider
          sx={{
            width: "15%",
                ...
                >
                  ...
                  >
                    {student.status === "تکمیل شده" ? (
                      <Box
                        onClick={() =>
                          handleEditProgramClick(student.username)
                        }
                        sx={{
                          display: "flex",
                          alignItems: "center",
                      ...
                      <Box
                        onClick={() =>
                          handleCreateProgramClick(student.username)
                        }
                        sx={{
                          display: "flex",
                          alignItems: "center",
